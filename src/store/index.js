...
export const store = new Vuex.Store({
  strict: process.env.NODE_ENV !== 'production',
    ...
    mutations: {
      ...
      SET_HEADER_PLACE(state) {
        state.page = 'place'
      }
    },
    actions: {
      setHeaderIndex({commit}) {
        commit('SET_HEADER_INDEX')
      },
      setHeaderGroup({commit}) {
        commit('SET_HEADER_GROUP')
      },
      setHeaderPlace({commit}) {
        commit('SET_HEADER_PLACE')
      }
    }
  })
